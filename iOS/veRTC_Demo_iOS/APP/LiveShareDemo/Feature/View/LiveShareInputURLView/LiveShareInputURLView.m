...
- (UITextField *)textField {
    if (!_textField) {
        ...
        NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:veString(@"please_enter_live_URL") attributes:@{NSForegroundColorAttributeName : [UIColor colorFromHexString:@"#86909C"]}];
...
- (UILabel *)horizontalLabel {
    if (!_horizontalLabel) {
        ...
        _horizontalLabel.text = veString(@"horizontal_screen");
    }
...
- (UILabel *)verticalLabel {
    if (!_verticalLabel) {
        ...
        _verticalLabel.text = veString(@"vertical_screen");
    }
...
- (UIButton *)watchButton {
    if (!_watchButton) {
        ...
        [_watchButton setTitle:veString(@"watch_live_together") forState:UIControlStateNormal];
